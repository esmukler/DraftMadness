...
RSpec.describe GamesController do
  context 'signed in as admin' do
    ...
    let!(:game) { FactoryGirl.create(:game, start_time: 3.hours.ago, next_game: next_game) }
    let(:next_game) { FactoryGirl.create(:game, school1_id: nil, school2_id: nil, round: 1) }
    let!(:other_previous_game) { FactoryGirl.create(:game, next_game: next_game) }

    before { sign_in(user) }

    describe 'POST /update_game' do
      it 'completes the game and advances the winning team' do
        patch :update, params: valid_game_update_params
      ...
      context 'other preivous game has lower id' do
        it 'adds winner as second school if other previous game has lower id' do
          ...
          patch :update, params: valid_game_update_params.merge(id: game.id)
  ...
  context 'signed in as non-admin user' do
    ...
    it 'returns 404' do
      expect { get :admin_games }.to raise_error(ActionController::RoutingError)
    end
  end
